export class POFClient {
  ...
  poiImageFile
    ...
    const allowedVideoExt = ['.mp4', '.mov'];

    if (!allowedVideoExt.some((ext) => videoName.endsWith(ext))) {
...
const docText = attrs.document_birth_date
  ? ` 证件出生日期 ${attrs.document_birth_date}，OCR年龄 ${attrs.ocr_age ?? '-'}。`
  : '';
  ...
  if (result.decision === 'approved') {
    return {
      type: 'success',
      title: '验证通过',
      message: `身份验证已通过。${scoreText}${docText}`
    };
  }
  ...
  if (result.decision === 'manual_review') {
    return {
      type: 'warning',
      title: '需人工审核',
      message: `系统无法自动确认，请进入人工审核流程。${scoreText}${docText}`
    };
  }
  ...
  return {
    type: 'error',
    title: '验证未通过',
    message: `${result.user_message || result.reason || '请重新录制视频后再试。'} ${scoreText}${docText}`
  };
}